  ...
  >
    <div class="max-w-5xl mx-auto flex items-center justify-between">
      <button 
        v-if="canGoBack"
        @click="handleBack"
        class="group flex items-center gap-1.5 font-black px-5 md:px-8 py-2.5 md:py-4 rounded-xl transition-all border border-white/20 shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95"
      ...
      <button 
        v-if="canGoNext"
        @click="handleNext"
        class="group flex items-center gap-2 font-black px-6 md:px-10 py-2.5 md:py-4 rounded-xl bg-primary text-white hover:bg-primary-hover shadow-xl shadow-primary/30 transition-all transform hover:-translate-y-0.5 active:scale-95 overflow-hidden relative border-b-2 border-black/10"
      >
