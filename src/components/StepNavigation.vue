...
<template>
  <div 
    class="fixed bottom-0 left-0 w-full p-8 pt-16 z-40 bg-gradient-to-t transition-colors duration-500"
    :class="{
  ...
  >
    <div class="max-w-5xl mx-auto flex items-center justify-between">
      <button 
        v-if="canGoBack"
        @click="handleBack"
        class="group flex items-center gap-2 font-black px-10 py-4 rounded-3xl transition-all border border-white/20 shadow-lg hover:shadow-xl hover:-translate-y-1 active:scale-95"
      ...
      <button 
        v-if="canGoNext"
        @click="handleNext"
        class="group flex items-center gap-2 font-black px-12 py-5 rounded-3xl bg-primary text-white hover:bg-primary-hover shadow-2xl shadow-primary/30 transition-all transform hover:-translate-y-1 active:scale-95 overflow-hidden relative border-b-4 border-black/10"
      >
